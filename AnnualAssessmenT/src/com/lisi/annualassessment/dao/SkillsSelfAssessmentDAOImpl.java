...
import com.lisi.annualassessment.controller.ExemptEmployeeInfoController;
...
public class SkillsSelfAssessmentDAOImpl implements SkillsSelfAssessmentDAO {
	...
	public List<SkillsSelfAssessment> getSkillsSelfAssessmentDetails(String clockNumber, String currentYear) {
		...
		System.out.println("clockNumber :: " + clockNumber + "currentYear :: " + currentYear + " ::: "
				+ Integer.parseInt(clockNumber));
	...
	@Override
	public int fetchYearOfAnSkillsSelfAssessment(int clockNum,String empYear) {
		...
		try {
			Query query = sessionFactory.getCurrentSession().createQuery(statement).setParameter(0, clockNum)
					.setParameter(1, empYear.trim());

			/*List<SkillsSelfAssessment> exempt = query.list();
			System.out.println("exempt.size() :: " + exempt.size());*/
			
			SkillsSelfAssessment skillSelfAssessmenT = (SkillsSelfAssessment)query.uniqueResult();
			
			if(skillSelfAssessmenT==null){
				
				return 0;
			}else{
				
				return Integer.parseInt(skillSelfAssessmenT.getAppraisalYear());
			}
