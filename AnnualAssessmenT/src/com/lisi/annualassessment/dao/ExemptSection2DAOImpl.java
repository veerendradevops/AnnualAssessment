...
public class ExemptSection2DAOImpl implements ExemptSection2DAO {
	...
	public void retriveAssessmentSummaryDatalogic(ExemptSection2 es, String year) {
		...
		try {
			if (!(year.isEmpty())||!(String.valueOf(year).equalsIgnoreCase("null"))) {
	...
	//	if ((!String.valueOf(year).isEmpty()) || (!String.valueOf(year).equalsIgnoreCase("null"))) {
	...
	public ExemptSection2 fetchObjectAchievementData(String clock, String year) {
		...
		query.setInteger(0, Integer.parseInt(clock));
		query.setString(1, year);
		
		ExemptSection2 exemptSection = (ExemptSection2)query.uniqueResult();
